...
In the CSV file, the columns are separated with , charcter , and the text quoted with " charcter. The csv data file loaded into R studio using read.csv2 function. 
...
```
The columns "Rating" , "Reviews", "Rating.Count", "Price" should be numerics. NA occurs upon conversion of character to numerics
due to erroreous data lines, the NA lines in dataset are removed from dataset: 
```{r store check, warning = FALSE,echo=TRUE}
