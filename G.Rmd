...
knitr::opts_chunk$set(tidy.opts = list(width.cutoff = 60), tidy = TRUE, warning = FALSE, echo = TRUE)
```
### Introduction to this project:

This is a project for the for the capstone project of HardvardX’s data science course. The project used R codes and Google PlayStore dataset to analyse the Ratings on Applications released on Google Store for download and instalation in Android mobile phones. 
...
Zipfile of the dataset is downloaded from GITHUB and unzipped. In the CSV file, the columns are separated with , charcter , and the text quoted with " charcter. The csv data file loaded into R studio using read.csv2 function. 

```{r store load, warning = FALSE,echo=TRUE}
...
```{r storeRatingTotal, warning = FALSE,echo=TRUE}
...
```{r store select, warning = FALSE,echo=TRUE}
...
```{r store split, warning = FALSE,echo=TRUE}
...
```{r store row,warning = FALSE, echo=FALSE }
...
```{r store 10 row, warning = FALSE,echo=FALSE }
...
```{r store zeros, warning = FALSE,echo=FALSE }
store %>% filter ( Rating == 0 ) %>% summarise (n=n() ) 

```
## Category

How many applications, ratings and mean rating by category in the store dataset:
```{r store countCategory,warning = FALSE, echo=FALSE }
...
```{r store plot-Category, warning = FALSE,echo=FALSE}
...
```{r store countDeveloper,warning = FALSE, echo=FALSE }
...
```{r store plot-Developer, warning = FALSE,echo=FALSE}
...
```{r store countContentRating,warning = FALSE, echo=FALSE }
...
```{r store plot-ContentRating,warning = FALSE, echo=FALSE}
...
```{r Mean,warning = FALSE, echo=FALSE}
...
```{r RMSE, warning = FALSE,echo=TRUE}
...
```{r Category-effect,warning = FALSE, echo=TRUE}

Category_avgs <- store %>% 
     group_by(Category) %>%  summarize(b_cat = mean(as.numeric(Rating) - store_mean))

Category_avgs %>% qplot(b_cat, geom ="histogram", bins = 10, data = ., color = I("black"))
...
```{r DeveloperId-effect,warning = FALSE, echo=TRUE}

Developer.Id_avgs <- store %>% select (Category, Developer.Id, Rating ) %>%
     left_join(Category_avgs, by='Category') %>%
     group_by(Developer.Id) %>%
     summarize(b_dev = mean(as.numeric(Rating) - store_mean - b_cat))
Developer.Id_avgs %>% qplot(b_dev, geom ="histogram", bins = 10, data = ., color = I("black"))
...
```{r ContentRating-avg, warning = FALSE,echo=TRUE}
...
Content.Rating_avgs <- store %>% select (Category, Developer.Id, Content.Rating, Rating ) %>%
     ...
     summarize(b_m = mean(as.numeric(Rating) - store_mean - b_cat - b_dev))
...
```{r reg-Category, warning = FALSE,echo=TRUE}

lambda <- 3
mu <- mean(as.numeric(store$Rating))
Category_reg_avgs <- store %>% 
    group_by(Category) %>% 
    summarize(b_cat = sum(as.numeric(Rating) - mu)/(n()+lambda), n_i = n()) 
data_frame(original = Category_avgs$b_cat, 
           regularlized = Category_reg_avgs$b_cat, 
...
```{r reg-Category-lambdas,warning = FALSE,echo=TRUE}
...
```{r reg-Developer-lambdas,warning = FALSE, echo=TRUE}

lambdas <- seq(0, 10, 0.25)

 Category_avgs2e <- store %>% select(Category) %>%  left_join(just_the_sum, by='Category') %>%     mutate(b_cat = s/(n_i+l1)) %>%
       select (Category , b_cat)
  Category_avgs2v <- validation %>% select(Category) %>%  left_join(just_the_sum, by='Category') %>%     mutate(b_cat = s/(n_i+l1)) %>%
       select (Category , b_cat)

u1 <- store %>% select ( Developer.Id, Rating) %>% cbind (Category_avgs2e$b_cat) %>% set_names ("Developer.Id","Rating","b_cat") %>% group_by(Developer.Id) %>%      summarize(s = sum( as.numeric(Rating) - mu - b_cat ), n_i = n()) %>% select(Developer.Id,s,n_i)


rmses6a <- sapply(lambdas, function(l){
    predicted_Ratings <- validation %>% select ( Developer.Id)  %>% 
      cbind (rmses5)%>%   set_names ("Developer.Id","mu_b_cat")%>%
      left_join(u1, by='Developer.Id') %>% 
        mutate(b_dev = s/(n_i+l)) %>%
...
```{r reg-ContentRating-lambdas, warning = FALSE,echo=TRUE}

lambdas <- seq(0, 10, 0.25)

 Developer.Id_avgs2 <- store %>% select(Developer.Id) %>%  left_join(u1, by='Developer.Id') %>%     mutate(b_dev = s/(n_i+l2)) %>%  select (Developer.Id , b_dev)

 rm(u1)

g1 <- store %>% select ( Content.Rating, Rating) %>% cbind (Category_avgs2e$b_cat,Developer.Id_avgs2$b_dev) %>% set_names ("Content.Rating","Rating","b_cat","b_dev") %>% group_by(Content.Rating) %>%      summarize(s = sum( as.numeric(Rating) - mu - b_cat - b_dev ), n_i = n()) %>% select(Content.Rating,s,n_i)


rmses7a <- sapply(lambdas, function(l){
    predicted_Ratings <- validation %>% select ( Content.Rating)  %>% 
      cbind (rmses5)%>%   set_names ("Content.Rating","mu_b_cat_bu")%>%
...
rmses7 <- validation %>% select (Content.Rating)  %>% 
      cbind (rmses6)%>%   set_names ("Content.Rating","mu_b_cat_bu")%>%
      left_join(g1, by='Content.Rating') %>% 
        mutate(b_CR = s/(n_i+l3)) %>%
        mutate(pred = mu_b_cat_bu +b_CR) %>%
        .$pred
...
```{r conclusion, warning = FALSE,echo=TRUE}
